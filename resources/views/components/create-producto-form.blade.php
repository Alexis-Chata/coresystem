<div x-data="{
...
}" class="relative mb-3 space-x-4 space-y-1.5">
    @can('create producto')
        ...
        <div x-show="open" @click.away="open = false"
            class="absolute z-10 mt-2 w-96 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
            <form wire:submit.prevent="createProducto" class="p-4">
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="name">
                        Nombre
                    </label>
                    <input wire:model="newProducto.name"
                ...
                <div class="mb-4">
                    ...
                    <select wire:model="newProducto.empresa_id"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="empresa_id">
                ...
                <div class="mb-4">
                    ...
                    <select wire:model="newProducto.marca_id"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="marca_id">
                ...
                <div class="mb-4">
                    ...
                    <select wire:model="newProducto.categoria_id"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="categoria_id">
                    ...
                    @error('newProducto.categoria_id')
                        <span class="text-red-500 text-xs italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select wire:model="newProducto.f_tipo_afectacion_id"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="f_tipo_afectacion_id">
                    ...
                    @error('newProducto.f_tipo_afectacion_id')
                        <span class="text-red-500 text-xs italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input wire:model="newProducto.porcentaje_igv"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="porcentaje_igv" type="number" step="0.01" placeholder="Porcentaje IGV">
                    @error('newProducto.porcentaje_igv')
                        <span class="text-red-500 text-xs italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select x-model="isCompuesto" wire:model="newProducto.tipo"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="tipo">
                ...
                <div class="mb-4" x-show="isCompuesto === 'estandar'">
                    ...
                    <input wire:model="newProducto.cantidad"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="cantidad" type="number" placeholder="Cantidad">
                    @error('newProducto.cantidad')
                        <span class="text-red-500 text-xs italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4" x-show="isCompuesto === 'estandar'">
                    ...
                    <input wire:model="newProducto.sub_cantidad"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="sub_cantidad" type="number" placeholder="Sub Cantidad">
                    @error('newProducto.sub_cantidad')
                        <span class="text-red-500 text-xs italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div x-show="isCompuesto === 'compuesto'" class="border-t mt-4 pt-4">
                    ...
                    <template x-for="(component, index) in components" :key="component.id">
                        <div class="mb-6">
                            ...
                            <div class="mb-2 w-full">
                                <x-searchable-select-with-stock :options="App\Models\Producto::where('tipo', 'estandar')
                                    ->get()
                                    ->map(function ($producto) {
                                        return ['id' => $producto->id, 'name' => $producto->name];
                                    })" :wire-model="'components'" :field="'component'"
                    ...
                    <div class="mb-4 mt-4">
                        ...
                        <input wire:model="newProducto.cantidad_total"
                            class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            id="cantidad_total" type="number" min="1"
                            :max="Math.min(...components.map(comp => comp.cantidad && comp.stock ? Math.floor(comp.stock /
                                comp
                                .cantidad) : Infinity))"
                            @input="
                        ...
                        @error('newProducto.cantidad_total')
                            <span class="text-red-500 text-xs italic">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select wire:model="newProducto.tipo_unidad"
                        class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="tipo_unidad">
                        <option value="NIU">NIU</option>
                        <option value="otro">Otro</option>
                    </select>
                    @error('newProducto.tipo_unidad')
                        <span class="text-red-500 text-xs italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between">
                    <button type="submit"
                        class="focus:ring bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                        Crear Producto
                    </button>
                </div>
            </form>
    ...
    @can('edit producto')
        <div x-show="showEditComponents" @click.away="showEditComponents = false"
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-2 w-full" x-data="{ component: component }">
                        <x-searchable-select-with-stock :options="App\Models\Producto::where('tipo', 'estandar')
                            ...
                            ->map(function ($producto) {
                                return ['id' => $producto->id, 'name' => $producto->name];
                            })" :selected="null" :wire-model="'editingComponents'"
                            :field="'component'" placeholder="Buscar producto..." />
                    </div>
                    ...
                    <div class="flex gap-4 items-center mt-2">
                        ...
                        <div class="flex-1">
                            <label class="block text-gray-700 text-sm mb-1">Subcantidad</label>
                            <input type="number" x-model="component.subcantidad" class="w-full border rounded px-3 py-2"
                                placeholder="Subcantidad" min="0">
                        </div>

                        <!-- Botón de eliminar -->
                        <button type="button" @click="removeEditingComponent(component.id)"
                            class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded text-sm transition-colors"
            ...
            <button type="button" @click="addEditingComponent"
                class="w-full bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded mt-2">
                + Agregar Producto
            </button>
            ...
            <div class="mb-4 mt-4">
                ...
                <input type="number" x-model="cantidadTotal"
                    @input="
                        ...
                        "
                    :max="Math.min(...editingComponents.map(comp => comp.cantidad && comp.stock ? Math.floor(comp.stock / comp
                        .cantidad) : Infinity))"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="edit_cantidad_total" min="1" placeholder="Cantidad total">
            </div>

            <div class="flex justify-end gap-2 mt-4">
                <button @click="showEditComponents = false"
    ...
    {{-- @can('delete producto') --}}
        <div class="mb-3 inline-block">
            <label class="text-gray-700 text-sm font-bold mb-2">Lista de Precio</label>
            <select
                class="focus:ring shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                wire:model.live="lista_precio_id">
                <option value="">-- Seleccione --</option>
                @foreach ($listasPrecio as $id => $name)
