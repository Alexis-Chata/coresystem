...
@php
    ...
    $fechaBusqueda = \Carbon\Carbon::parse($fecha)->format('d-m-Y');

    // Debug
    \Log::info('Buscando pedidos para la fecha:', ['fecha_busqueda' => $fechaBusqueda]);

    if ($user->hasRole('admin')) {
        $pedidosPorVendedor = \App\Models\Pedido::where('fecha_emision', $fechaBusqueda)
    ...
    \Log::info('Pedidos encontrados:', [
        'cantidad' => $pedidosPorVendedor->count(),
        'fecha_emision_primer_pedido' => $pedidosPorVendedor->first()?->first()?->fecha_emision
    ]);
@endphp
<style>
            main > :first-child{
...
<div
    x-data="{
        ...
        actualizarFecha(nuevaFecha) {
            ...
            this.$dispatch('fecha-changed', nuevaFecha);
        }
    }"
    class="cont_reporte space-y-6 bg-white dark:bg-gray-900 rounded-lg shadow-md"
>
    ...
    <header class="flex justify-between items-center">
        ...
        <input
            type="date"
            x-model="fecha"
            @change="actualizarFecha($event.target.value)"
