    ...
    <div x-show="open" @click.away="open = false" class="absolute z-10 mt-2 w-96 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
        <form wire:submit.prevent="createRuta" class="p-4">
            <div class="mb-4">
                ...
                <input wire:model="newRuta.codigo" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="codigo" type="text" placeholder="Código de la ruta">
                @error('newRuta.codigo') <span class="text-red-500 text-xs italic">{{ $message }}</span> @enderror
            </div>
