<div class="relative" x-data="{ isOpen: false }" x-on:dropdown-opened.window="isOpen = true">
    <div class="relative">
        <input type="text" wire:model.live="search" @focus="isOpen = true; $wire.showAll()" @click.away="isOpen = false"
        ...
        @if ($search)
            <button wire:click="clearSelection" @click="isOpen = true"
    ...
    @if ($vendedor_id && $showDropdown && count($clientes) > 0)
        <div x-show="isOpen" class="absolute z-50 w-full mt-1 bg-white rounded-md shadow-lg max-h-60 overflow-auto">
            @forelse($clientes as $cliente)
                @php
                    // 1) Marcas que cumplen cobertura dentro de comprobantes del mes actual + estado_reporte=1
                    $marcasCobertura = collect();

                    $comps = $cliente->fComprobanteSunats ?? collect(); // ajusta si tu relación se llama distinto

                    foreach ($comps as $comp) {
                        $detalles = $comp->detalle ?? collect(); // ajusta si se llama detalles

                        foreach ($detalles as $det) {
                            $marca = $det->producto?->marca;

                            if ($marca && (int) $marca->resaltar_cobertura === 1) {
                                $marcasCobertura->push($marca);
                            }
                        }
                    }

                    $marcasCobertura = $marcasCobertura->unique('id')->values();
                @endphp

                <div wire:key="cliente-{{ $cliente->id }}" wire:click="selectCliente({{ $cliente->id }})"
                    class="px-4 py-2 cursor-pointer hover:bg-gray-100 flex items-center justify-between gap-3">
                    <div class="min-w-0">
                        <span class="text-gray-600">#{{ $cliente->id }}</span>
                        - {{ $cliente->razon_social }}
                        - {{ $cliente->listaPrecio->name }}

                        {{-- ✅ puntitos de marcas --}}
                        @if ($marcasCobertura->isNotEmpty())
                            <span class="inline-flex items-center gap-1 ml-2 align-middle">
                                @foreach ($marcasCobertura->take(4) as $m)
                                    @php
                                ...
                                @if ($marcasCobertura->count() > 4)
                                    <span class="text-xs text-gray-400">+{{ $marcasCobertura->count() - 4 }}</span>
                                @endif
                            </span>
                        @endif
                    </div>
