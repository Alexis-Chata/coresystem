...
class ImprimirComprobante extends Component
{
    ...
    public function imprimir()
    {
        ...
        try {
            ...
            $comprobante = FComprobanteSunat::with(['vendedor', 'tipo_doc', 'cliente.padron', 'conductor', 'detalle.producto'])->find(1);
            ...
            $printer->feed();
            $printer->text("RUTA: " . str_pad($comprobante->ruta_id, 4, "0", STR_PAD_LEFT) . "  SEC.: " . str_pad($comprobante->padron->nro_secuencia, 5, "0", STR_PAD_LEFT));
            $printer->feed();
            ...
            foreach ($comprobante->detalle as $detalle) {
                $printer->text(strtoupper(str_pad($detalle->codProducto, 5, "0", STR_PAD_LEFT) . " " . substr($detalle->descripcion, 0, 35)));
                $printer->feed();
